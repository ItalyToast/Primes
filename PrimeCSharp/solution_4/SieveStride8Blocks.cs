...
namespace PrimeSieveCS
{
    ...
    {
        const int blocksize = 0x8000; //32k blocksize in bytes, that should fit in most processors L1 cache
        readonly uint sieveSize = 0;
        readonly uint halfLimit;
        readonly ulong[] bits;

        public string Name => "italytoast-stride8-blocks32k";
        ...
        unsafe static void ClearBitsStride8Blocks(byte* ptr, uint start, uint factor, uint limit)
        {
            ...
            while (blockStart <= bytecount)
            {
                for (int stride = 0; stride < 8; stride++)
                {
                    ...
                    var blockEnd = Math.Min(bytecount, index + blocksize);

                    uint i;
                    for (i = index; i < blockEnd; i += factor)
                    {
                        ptr[i] |= mask;
                    }
                    strides[stride] = (i, mask);
                }
